...
### 4. 🛡️ AI 개인정보 보호 (차량 번호판 & 안면 정밀 블러)
...
### 🛡️ 1. [NEW] AI 개인정보 보호 (차량 번호판 & 안면 블러)
- **차량 영역(Vehicle ROI) 선행 탐지**: YOLOv8 모델로 차량을 우선 감지하여 배경 오탐 원천 차단
- **한국 번호판 규격 검증**: 번호판 색상 마스크 및 일렬 숫자 시퀀스 기하학적 검증
- **초고속 안면 인식**: YuNet 딥러닝 기반 보행자 및 운전자 얼굴 자동 블러
- **모션 트래킹 & 궤적 보간**: 지연 없는 밀착 추종 및 60fps 프리뷰/고속 렌더링 최적화
