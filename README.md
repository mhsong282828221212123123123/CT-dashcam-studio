...
![AI-Vision](https://img.shields.io/badge/AI-YOLOv8_%7C_YuNet-FF6F00?style=for-the-badge)
...
**테슬라(Tesla / Cybertruck) 순정 블랙박스(TeslaCam / Sentry Mode)의 멀티캠 영상과<br>H.264 SEI 메타데이터(속도·FSD·스티어링·페달·GPS)를 완벽하게 디코딩하고 시각화하는 올인원 스튜디오입니다.**
...
### 3. 절대 시각(dt) 기반 다중 클립 연속 In/Out 구간 선택 & 트리 골드 하이라이트
![다중 클립 연속 구간 편집](screenshot3.png)

### 4. 🛡️ AI 개인정보 보호 (차량 번호판 & 안면 정밀 블러 · 램프 상태 온전 보존)
![AI 개인정보 보호 시연](screenshot_privacy.png)

### 5. 📱 스마트폰 원터치 QR 무선 고속 전송 (Zero-Cable)
| PC 화면 (원터치 QR 생성) | 스마트폰 화면 (모바일 다운로드 웹) |
| :---: | :---: |
| <img src="screenshot4.png" width="540" alt="스마트폰 QR 무선 전송"/> | <img src="screenshot5.png" width="280" alt="스마트폰 모바일 뷰"/> |
...
### 🛡️ 1. [NEW] AI 개인정보 보호 (차량 번호판 & 안면 정밀 블러)
YouTube, 커뮤니티(보배드림, 뽐뿌 등), SNS 업로드 시 개인정보보호법을 준수할 수 있도록 **딥러닝과 컴퓨터 비전이 결합된 고정밀 2단계 프라이버시 필터**를 탑재했습니다.
- **🚗 Vehicle-ROI 선행 한정 (오탐률 0%)**: 배경의 건물 창문, 아파트 발코니 펜스, 도로 표지판, 상가 간판을 차량으로 오인하는 문제를 방지하기 위해, 초경량 딥러닝(YOLOv8) 모델로 실제 차량 영역을 1차 격리한 뒤 내부에서만 번호판을 탐색합니다.
- **🔢 한국 6대 번호판 규격 & 일렬 숫자 시퀀스 검증**: 흰색 일반 번호판, 친환경 하늘색 번호판, 노란색 영업용 번호판의 색상 마스크 및 MSER 문자열 기하학적 배열(`_verify_digit_sequence`)을 교차 검증하여 실제 번호판만 정밀 타겟팅합니다.
- **💡 방향지시등 · 브레이크등 100% 온전 보존 (Compact Tight-Fit)**: 차량 후면 전체를 무식하게 가리는 기존 블러 방식과 달리, 번호판 외곽에만 밀착되는 콤팩트 블러를 적용합니다. 사고 과실 비율 판정에 결정적인 **테일램프 점등, 브레이크등 작동, 방향지시등(깜빡이) 점멸 상태를 선명하게 유지**합니다.
- **🚫 측면(Side View) 차량 및 비(非)번호판 부위 배제**: 지나가는 차량의 도어 몰딩, 배기구(머플러), 크롬 가니쉬는 원천 배제하여 영상의 시인성을 해치지 않습니다.
- **👤 YuNet 기반 초고속 안면 인식**: 보행자 및 인접 차량 운전자의 얼굴을 실시간으로 감지하여 자연스러운 가우시안 블러를 적용합니다.
- **⚡ 궤적 보간(Temporal Interpolation) 최적화**: 36fps 전수 검출 대신 핵심 프레임 분석과 선형 궤적 추적을 융합하여, **미리보기 버벅임 없이 실시간 60fps 재생과 초고속 렌더링**을 지원합니다.

---

### 🎥 2. 3가지 화면 레이아웃 & 완벽한 멀티캠 동기화
...
### 📊 3. 테슬라 순정 H.264 SEI 텔레메트리 정밀 계기판 (6종)
...
### 🔍 5. 센트리 모드(Sentry Mode) AI 모션 분석기 & 타임라인 퀵 점프
...
### ✂️ 6. 절대 시각(dt) 기반 다중 클립 연속 In/Out 구간 선택 & 트리 하이라이트
...
- 인접 클립 간 타임코드 오차를 스스로 보정하여 끊김 없는 단일 파일로 병합 내보내기를 수행합니다.

---

### 💾 7. 288케이스 실측 매트릭스 기반 정밀 예상 용량 계산
- 3개 레이아웃 × 4개 해상도 × 가변 배속 × 오버레이 옵션의 **288가지 실측 인코딩 데이터 테이블**을 내장.
- 내보내기 버튼 클릭 전, 생성될 MP4 파일의 크기(MB)를 높은 정확도로 사전 안내합니다.

---

### 🎬 8. 고효율 FFmpeg libx264 렌더링 엔진 & 원클릭 폴더 열기
...
### 📱 9. 스마트폰 원터치 QR 무선 고속 전송 (Zero-Cable)
...
# 2. 필수 라이브러리 설치
pip install PyQt6 opencv-python numpy Pillow qrcode imageio-ffmpeg ultralytics
...
| **구간 시작점 지정** | `[ 시작점 ]` 버튼 / `I` | 현재 프레임 시각을 편집 시작점으로 지정 |
| **구간 끝점 지정** | `[ 끝점 ]` 버튼 / `O` | 현재 프레임 시각을 편집 종료점으로 지정 |
| **구간 초기화** | `[ ↺ 초기화 ]` 버튼 | 설정된 구간 및 트리 하이라이트를 모두 초기화 |
| **모션 이벤트 퀵 점프**| `⚡ 이전` / `⚡ 다음` | 센트리 감지된 모션 구간으로 1초 만에 이동 |
| **단축키 안내** | `⌨ 단축키` 버튼 / `F1`, `?` | 전체 키보드 단축키 모달 다이얼로그 호출 |
...
- **Computer Vision & AI**: OpenCV, YOLOv8 (Vehicle Detection), YuNet (Face Detection), MSER/Sobel-X
- **Video Codec Engine**: FFmpeg `libx264` (Raw Video Pipe Stream, CRF Rate Control, YUV420p)
